...
/**
 * Make a chapter the active choice point: reopen it, clear any stale close
 * state from a prior round, and keep universe statuses consistent.
 * Shared by /api/admin/activate (explicit pick) and /api/admin/advance
 * (next-in-sequence after NFT distribution).
 */
export async function activateChoicePoint(choice_point: string): Promise<void> {
